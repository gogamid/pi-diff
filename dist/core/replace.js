...
function getIndentAdjustment(expectedLines, actualLines) {
    let adjustment;
    for (let i = 0; i < expectedLines.length; i++) {
        if (expectedLines[i].trim() === "")
            continue;
        ...
        const current = actual === expected
            ...
            : actual.endsWith(expected)
                ? { kind: "add", whitespace: actual.slice(0, actual.length - expected.length) }
                : expected.endsWith(actual)
                    ? { kind: "remove", whitespace: expected.slice(0, expected.length - actual.length) }
                    : undefined;
        if (!current)
            return undefined;
        if (!adjustment) {
        ...
        else if (adjustment.kind !== current.kind ||
...
function applyIndentAdjustment(newText, adjustment, lineEnding) {
    const lines = newText.split(/\r?\n/);
    const adjusted = lines.map((line) => {
        if (line.trim() === "" || adjustment.kind === "none")
            return line;
        if (adjustment.kind === "add")
            return adjustment.whitespace + line;
        return line.startsWith(adjustment.whitespace) ? line.slice(adjustment.whitespace.length) : undefined;
    });
    return adjusted.every((line) => line !== undefined) ? adjusted.join(lineEnding) : undefined;
}
