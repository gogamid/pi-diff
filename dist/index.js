 ...
 *  Also auto-switches the Shiki syntax theme when the pi theme changes between light and dark. */
function resolveDiffColors(theme) {
    ...
    if (_lastResolvedThemeKey && _lastResolvedThemeKey !== currentThemeKey) {
        // Theme changed — reset for re-derivation
...
function summarizeThemed(a, d, theme) {
...
export default async function diffRendererExtension(pi) {
    ...
    function stashEditHeaderStats(toolCallId, edits, diffLines, added, removed) {
        if (!toolCallId)
            return;
        editHeaderStatsByCallId.set(toolCallId, { edits, diffLines, added, removed });
    }
    const writeHeaderStatsByCallId = new Map();
    function stashWriteHeaderStats(toolCallId, added, removed) {
        if (!toolCallId)
            return;
    ...
    function setToolErrorBg(text, theme) {
        ...
        text.customBgFn = (line) => injectBg(line, [], background, background);
    }
    ...
    function editCallStatsSuffix(toolCallId, theme) {
