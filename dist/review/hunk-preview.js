...
function loadShikiThemeMap() {
    try {
        const raw = loadDiffConfig();
        const st = raw.diffShikiThemes;
        return {
            light: (st?.light ?? "github-light"),
            dark: (st?.dark ?? "github-dark"),
        };
    }
    catch {
...
export async function renderReviewHunkPreview(input) {
    ensurePalette();
...
export function themeCacheKey(theme) {
    if (!theme?.fg)
        return "no-theme";
    const fgKeys = [
        "toolTitle",
...
function detectThemeMode(theme) {
    if (theme?.getBgAnsi) {
        try {
...
function shouldUseSplit(diff, width, maxRows, options = {}) {
...
export async function renderUnified(diff, language, maxLines, colors, width, options = {}) {
    if (!diff.lines.length)
        return "";
...
export async function renderSplit(diff, language, maxLines, colors, width, options = {}) {
    ...
    for (const row of visible) {
        ...
        const leftHalf = buildHalf(row.left, leftHighlight, isPairedChange && wordDiffBalanced && wordDiff.similarity >= WORD_DIFF_MIN_SIM ? wordDiff.oldRanges : null, "left");
        const rightHalf = buildHalf(row.right, rightHighlight, isPairedChange && wordDiffBalanced && wordDiff.similarity >= WORD_DIFF_MIN_SIM ? wordDiff.newRanges : null, "right");
        ...
        for (let rowIndex = 0; rowIndex < maxRows; rowIndex++) {
            ...
            if (!leftHalf.gutter && !rightHalf.gutter && !leftBody && !rightBody)
                continue;
            output.push(`${rowIndex === 0 ? leftHalf.gutter : leftHalf.continuation}${leftBody.trimEnd()}${rowIndex === 0 ? rightHalf.gutter : rightHalf.continuation}${rightBody.trimEnd()}`);
        }
    }
